...
- [~] muotikaupat — vaatteiden verkkokaupat aikuisille (ei lasten, se on jo tehty). Kandidaatit: Zalando (zalando.fi), Boozt (boozt.com), H&M (hm.com/fi), KappAhl (kappahl.com/fi-fi), Cubus (cubus.fi), Lindex (lindex.com/fi). 6+ itsenäistä toimijaa omilla fi-sivustoilla. Tarkista SAME_COMPANY: H&M Group omistaa H&M+COS+Monki+Weekday — laske vain yksi; Inditex omistaa Zara+Mango+Massimo Dutti — laske vain yksi.
- [ ] sahkoautojen-lataus — sähköauton julkiset latauspalvelut Suomessa. Kandidaatit: Virta (virta.fi, suomalainen), Recharge/Plugit (plugit.fi), ABC Lataus (abcasema.fi, S-ryhmä), Fortum Charge & Drive (fortum.com/fi/sahkoa), K-Lataus (k-lataus.fi tai kesko.fi-alaisuus), IONITY (ionity.eu). 6 operaattoria. Mittaa: hinnoittelu (kWh/min), latausverkko koko, mobiilisovellus, kausimaksuvaihtoehdot, asiakaspalvelu.
- [ ] lomamokkivuokraus — mökkivuokrauksen verkkoalustat. Kandidaatit: Lomarengas (lomarengas.fi, Suomen suurin, per. 1967), Nettimökki (nettimokki.com), Airbnb (airbnb.fi), Booking.com (booking.com/fi), Huvila.net (huvila.net). Tarvitaan 6. toinen — tarkista Cottages.fi, Mökkivuokra.fi tai Loma-Hytti.fi olemassaolo ennen buildaamista. Mittaa: kohteiden maara FI, hintalapinakyyvyys, varauksen helppous, peruutusehdot, asiakaspalvelu.
- [ ] autotarvikkeet-verkossa — autovaraosien ja -tarvikkeiden verkkokaupat. Kandidaatit: Nettivaraosa (nettivaraosa.fi), Motonet (motonet.fi), Autodoc (autodoc.fi), Biltema (biltema.fi), Bilema (bilema.fi), Mekonomen (mekonomen.fi). 6 itsenäistä toimijaa. Ero olemassa oleviin kategorioihin: autokorjaamot = huoltopalvelut, rautakaupat = rakennustarvikkeet — tama on puhtaasti autoosien/tarvikkeiden webshop-vertailu.
- [ ] rahansiirtopalvelut — kansainvaliset rahansiirtopalvelut kuluttajille. Kandidaatit: Wise (wise.com/fi), Revolut (revolut.com/fi), Western Union (westernunion.com/fi), MoneyGram (moneygram.com/fi), Paysend (paysend.com/fi), Skrill (skrill.com). 6 palvelua. Mittaa: siirtokustannukset (eurot/prosentti), valuuttakurssimarginaali, nopeus, lapinakyyvyys, mobiilisovellus. Tarkista ero pankit-kategoriaan: pankit = talletuspankki+laajapalvelu; tama = puhdas rahansiirto.
