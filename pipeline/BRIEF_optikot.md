# Extraction task — vertical `optikot`
...
WHAT THIS CATEGORY MEASURES: Finnish optician chains. Measures the PUBLIC site: are glasses/lens prices and eye exam prices stated, can you book online, are stores listed?
It does NOT measure service quality. Never score anything from inside a logged-in area.

Write EXACTLY ONE file: pipeline/extracts/optikot__<slug>.json — nothing else.

JSON shape (all keys required):
{
 "slug": "<slug>",
 "fetched_ok": ["<every URL you actually loaded and read>"],
 "silmalasien_hinnat_esilla": "kylla|osittain|ei",    // Silmälasien/linssien hintatietoja julkisesti esillä
 "nakotarkastuksen_hinta": "kylla|osittain|ei",    // Näöntarkastuksen hinta kerrottu
 "ajanvaraus_verkossa": "kylla|osittain|ei",    // Ajanvaraus verkossa ilman kirjautumista
 "liikkeet_ja_aukioloajat": "kylla|osittain|ei",    // Liikkeet ja aukioloajat kerrottu
 ...
 "naontarkastus_hinta_eur": <number|null>,   // advertised price of a standard eye exam (näöntarkastus). 0 if advertised as free. null if not stated.
