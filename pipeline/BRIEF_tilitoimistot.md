# Extraction task — vertical `tilitoimistot`
...
WHAT THIS CATEGORY MEASURES: Finnish national accounting firm chains. Measures the PUBLIC site: is pricing (monthly packages or hourly rates) public, are services described, are software/integrations and authorisations (Taloushallintoliitto, KLT) mentioned?
It does NOT measure service quality. Never score anything from inside a logged-in area.

Write EXACTLY ONE file: pipeline/extracts/tilitoimistot__<slug>.json — nothing else.

JSON shape (all keys required):
{
 "slug": "<slug>",
 "fetched_ok": ["<every URL you actually loaded and read>"],
 "hinnat_esilla": "kylla|osittain|ei",    // Hinnoittelu (kuukausipaketit tai tuntihinnat) julkisesti esillä
 "palvelut_kuvattu": "kylla|osittain|ei",    // Palvelujen sisältö kuvattu
 "ohjelmistot_kerrottu": "kylla|osittain|ei",    // Käytetyt ohjelmistot ja integraatiot kerrottu
 "auktorisointi_kerrottu": "kylla|osittain|ei",    // Auktorisointi tai KLT-pätevyydet kerrottu
 ...
 "kk_hinta_alkaen_eur": <number|null>,   // cheapest advertised monthly package price in EUR/kk. null if not publicly stated.
