# Extraction task — vertical `autokatsastus`
...
WHAT THIS CATEGORY MEASURES: Finnish vehicle inspection chains. Measures the PUBLIC site: can a car owner find the inspection price, book with the price visible, and find stations?
It does NOT measure service quality. Never score anything from inside a logged-in area.

Write EXACTLY ONE file: pipeline/extracts/autokatsastus__<slug>.json — nothing else.

JSON shape (all keys required):
{
 "slug": "<slug>",
 "fetched_ok": ["<every URL you actually loaded and read>"],
 "katsastushinta_esilla": "kylla|osittain|ei",    // Määräaikaiskatsastuksen hinta julkisesti esillä
 "ajanvaraus_hinnalla": "kylla|osittain|ei",    // Nettiajanvaraus näyttää hinnan ennen yhteystietoja
 "asemat_ja_aukioloajat": "kylla|osittain|ei",    // Asemat ja aukioloajat kerrottu
 "palveluvalikoima_kuvattu": "kylla|osittain|ei",    // Katsastuslajit ja lisäpalvelut hintoineen kuvattu
 ...
 "maaraaikaiskatsastus_hinta_eur": <number|null>,   // cheapest advertised price for a standard passenger-car periodic inspection (määräaikaiskatsastus). null if not publicly stated.
