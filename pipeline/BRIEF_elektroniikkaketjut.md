# Extraction task — vertical `elektroniikkaketjut`
...
WHAT THIS CATEGORY MEASURES: consumer electronics retail chains operating in Finland.
Measured from the BUYER's perspective on the public site: are product prices visible,
are delivery methods and costs stated before checkout, are return and warranty terms
publicly available, are stores/pickup points and opening hours listed (a pure webshop
is scored on its pickup/delivery info)? Does NOT compare product prices between chains.

Write EXACTLY ONE file: pipeline/extracts/elektroniikkaketjut__<slug>.json — nothing else.
...
JSON shape (all keys required):
{
 ...
 "myymalat_ja_aukioloajat": "kylla|osittain|ei",   // Myymälät/noutopisteet ja aukioloajat kerrottu
 "y_tunnus_esilla": "kylla|osittain|ei",           // Y-tunnus esillä (ulkomaisella: yhtiö selkeästi kerrottu)
